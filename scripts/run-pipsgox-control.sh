...
if [[ -f "$PIDFILE" ]] && kill -0 "$(cat "$PIDFILE" 2>/dev/null)" 2>/dev/null; then
  echo "STATUS: PASS — Dev Control already running (PID $(cat "$PIDFILE"))"
  exit 0
fi
...
(
  cd "$ROOT" || exit 1
  exec env PIPSGOX_CONTROL_PORT="$PORT" python backend/dev_server.py
) >"$LOGFILE" 2>&1 &
...
for _ in {1..15}; do
  ...
  if ! kill -0 "$PID" 2>/dev/null; then
    echo "STATUS: FAIL — Dev Control exited during startup."
    tail -n 80 "$LOGFILE"
    rm -f "$PIDFILE"
    exit 1
  fi
...
echo "STATUS: FAIL — Dev Control did not become ready."
tail -n 80 "$LOGFILE"
exit 1
