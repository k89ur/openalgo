...
start() {
  ...
  if [[ -f "$pidfile" ]] && kill -0 "$(cat "$pidfile")" 2>/dev/null; then
    ...
    rm -f "$pidfile"
    echo "Starting $name..."
    PIPSGOX_WEB_URL="$WEB_URL" FYERS_REDIRECT_URI="$FYERS_CALLBACK" \
...
start "Backend" "$RUN_DIR/backend.pid" "$RUN_DIR/backend.log" \
  "PIPSGOX_WEB_URL='$WEB_URL' FYERS_REDIRECT_URI='$FYERS_CALLBACK' cd backend && python -m uvicorn app.main:app --host 0.0.0.0 --port 8000" \
  "8000" "http://127.0.0.1:8000/health"

start "Frontend" "$RUN_DIR/frontend.pid" "$RUN_DIR/frontend.log" \
...
if grep -q '"configured":true' <<<"$FYERS_STATUS"; then
  if grep -q '"connected":true' <<<"$FYERS_STATUS"; then
    ...
    echo "FYERS: login required — PIPSGOX will open the FYERS login automatically."
  fi
else
